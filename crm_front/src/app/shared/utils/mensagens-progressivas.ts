...
export interface EtapaFunilProgressivo {
  ponto: PontoDisparoAutomacao;
  rotulo: string;
  descricao: string;
  grupo: 'lembrete' | 'cobranca';
}

export const ETAPAS_FUNIL_PROGRESSIVO: EtapaFunilProgressivo[] = [
  ...CRONOGRAMA_LEMBRETES_AUTOMACAO.map((item) => ({
    ponto: `LEMBRETE_D${item.dias}` as PontoDisparoAutomacao,
    rotulo: item.rotulo,
    descricao:
      item.dias >= 3
        ? 'Tom amigável, PIX disponível, sem pressão.'
        : item.dias === 1
          ? 'Segundo lembrete, tom mais direto, PIX atualizado.'
          : 'Mensagem objetiva: hoje é o dia do vencimento.',
    grupo: 'lembrete' as const,
  })),
  ...CRONOGRAMA_COBRANCAS_AUTOMACAO.map((item) => ({
    ponto: `COBRANCA_D${item.dias}` as PontoDisparoAutomacao,
    rotulo: item.rotulo,
    descricao:
      item.dias === 7
        ? 'Tentativa de recuperar o cliente.'
        : 'Cobrança progressiva com aviso de acesso suspenso.',
    grupo: 'cobranca' as const,
  })),
];
...
export function parsearMensagensProgressivas(
  ...
  const pontosValidos = new Set(Object.keys(MENSAGENS_PADRAO_POR_PONTO));
  const resultado: MensagensProgressivasMap = {};

  for (const [chave, conteudo] of Object.entries(bruto)) {
    if (!pontosValidos.has(chave) || typeof conteudo !== 'string') continue;
...
export function serializarMensagensProgressivas(
  mapa: Record<PontoDisparoAutomacao, string>
): string {
